...
const Events = () => {
    const {events,isLoading,publishevent,removeEvent} = useEvents();
...
if(!isLoading){
        ...
        <div className="mx-10 mt-10 text-lg flex flex-col gap-5">
            ...
            <Link className="inline-flex self-start gap-2 px-3 py-2 font-mono rounded-md font-bold text-slate-50 bg-green-500 hover:bg-green-800" to={"/subadmin/dashboard/events/create"}><FilePlus />Create a new Event</Link>
                    ...
                    currentArticles.map((val, index) => (
                        <tr key={index} className="odd:bg-blue-50">
                            ...
                            <td className="p-4 flex gap-1 mt-4">
                                {/* publish */}
                                {
                                    val.published_at === null?<button type="button" title='Publish'><BookPlus onClick={()=>publishevent(val.id)} color='green' size={30}/></button>:null
                                }
                                
                                {/* Edit */}
                                <Link to={`/subadmin/dashboard/events/${val.id}/update`} title='Edit' className="inline">
                                    <FilePenLine color='blue' size={30}/>
                                </Link>
                                <button title="Delete" onClick={()=>removeEvent(val.id)}>
